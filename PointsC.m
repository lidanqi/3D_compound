function [ list ] = PointsC( requiredlevel )
...
for i=0:level
    for j=0:(level-i)
        ...
        points = (4*2^i+1)*(2*2^j+1)*(2*2^k+1);
...
fullpoints=(2^requiredlevel*4+1)*(2^requiredlevel*2+1)*(2^requiredlevel*2+1);
sparsepoints_max = max(list(:,4));
